...
def partTwo():
    ...
    for pos, (_, tile) in enumerate(arrangement):
        ...
        img.update({(x + xOffset - 1, y + yOffset - 1) for x, y in tile - border})
    ...
    print("seaMonsterTemplate:", seaMonsterTemplate)
    for variant in imageVariants(img, imageWidthPx):
        print("Trying this composed image variant:")
        for y in range(imageWidthPx):
            for x in range(imageWidthPx):
                sys.stdout.write('#' if (x, y) in variant else '.')
            sys.stdout.write('\n')
        ...
        for y in range(imageWidthPx):
            for x in range(imageWidthPx):
                ...
                if x == 2 and y == 2:
                    for c in seaMonster:
                        if c not in img:
                            print("HM! {},{} is in the sea monster, but not in the image. Pity.".format(c[0],c[1]))
                if seaMonster.issubset(img):
...
if __name__ == '__main__':
    ...
    arrangement = arrange([])
    print([tileId for tileId, tile in arrangement])
